 ...
 */
@ActionDescriptor(
        name = "WAAE-application-on-hold",
        description = "WAAE sendevent start-job through webservice",
        category="Workload." )
    public class WAAEapplicationonhold extends Action {
    private static final long serialVersionUID = 1L;
    private static final Logger log = Logger.getLogger(WAAEapplicationonhold.class);
    ...
    public ActionResult execute() {
        ...
        try {
            log.debug("A call was received to execute a Restful Action: " + "WAAE-application-on-hold");

            String rawRequestValue = "{\"application\":\"${application}\",\"comment\":\"${comment}\"}";

            response = new RestClientManager().restCall(HttpMethod.POST, authType, getInput(),
                "${WAAE Web Server}/AEWS/event/hold-job", 30, "", "application/json",
            ...
            response = new HttpRDKResponse("There was an error trying to execute a REST Call Action: '"
                + "WAAE-application-on-hold" + "' for Action Pack: "   + "", null, -1);
    ...
    private ActionResult getActionResult() {
        ...
        StringBuilder errors = new StringBuilder();
                                                                        if( ErrorHelper.testErrorCondition(responsecode3, Operations.valueOf("GREATER_THAN"), "399")) {
                                            try { 
                                errors.append( ReplacementsHelper.replaceOutputParameters( ReplacementsHelper.replaceInputParameters( "Response Code: ${Response Code}" , getInput() ), getOutput() ) ).append("<br/>");
            } catch(Exception e) {
                log.error("There was an error on the replacement of parameters", e);
            }           
                        }
