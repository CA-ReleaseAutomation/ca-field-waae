...
@ActionDescriptor(
        name = "WAAE-reply-job",
        description = "WAAE sendevent reply-job through webservice",
        category="Workload." )
    public class WAAEreplyjob extends Action {
    private static final long serialVersionUID = 1L;
    private static final Logger log = Logger.getLogger(WAAEreplyjob.class);
    ...
    public ActionResult execute() {
        ...
        try {
            log.debug("A call was received to execute a Restful Action: " + "WAAE-reply-job");

            String rawRequestValue = "{\"jobName\":\"${jobname}\",\"response\":\"${response}\"}";

            response = new RestClientManager().restCall(HttpMethod.POST, authType, getInput(),
                "${WAAE Web Server}/AEWS/event/reply-job", 30, "", "application/json",
            ...
            response = new HttpRDKResponse("There was an error trying to execute a REST Call Action: '"
                + "WAAE-reply-job" + "' for Action Pack: "   + "", null, -1);
    ...
    private List<InputParam> getInput() {
            ...
            inputParameters.add(p);     
                    p = new InputParam();
            p.setName("response");
                                    p.setVariableName("response");
            p.setPrefix("");
            p.setType(VarType.String);
                                                                     p.setValue(response5);
